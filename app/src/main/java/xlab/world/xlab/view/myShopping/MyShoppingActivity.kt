...
class MyShoppingActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        myShoppingViewModel.setResultCode(resultCode = resultCode)
        when (resultCode) {
            Activity.RESULT_OK -> {
                when (requestCode) {
    ...
    private fun observeViewModel() {
        ...
        myShoppingViewModel.orderCancelEventData.observe(owner = this, observer = android.arch.lifecycle.Observer { eventData ->
            eventData?.let { isSuccess ->
                if (isSuccess) {
                    myShoppingViewModel.setResultCode(resultCode = Activity.RESULT_OK)
        ...
        v?.let {
            when (v.id) {
                R.id.actionBackBtn -> { // 뒤로가기
                    myShoppingViewModel.backBtnAction()
                }
